...
X_test_copy = list(X_test)
y_test_series = pd.Series(y_test_copy, name='y_test')
x_test_series = pd.Series(X_test_copy, name='X_test')

y_test_series.to_csv('y_test_series_4.csv',  header=True, index=False)
x_test_series.to_csv('x_test_series_4.csv',  header=True, index=False)

label_map = ['angry', 'sad', 'surprised', 'happy']
num_class=4

def model4(num_class): 
    model = Sequential()

    # 1 - Convolution
    model.add(Conv2D(64,kernel_size=3, padding='same', input_shape=(144, 144,1), kernel_initializer='he_normal'))
    ...
    # 5th Convolution 
    model.add(Conv2D(512,kernel_size=3, padding='same', kernel_initializer='he_normal'))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    model.add(Conv2D(512,kernel_size=3, padding='same', kernel_initializer='he_normal'))
    model.add(BatchNormalization())
...
def plot_results(history):
    ...
    plt.subplot(1, 2, 1)
    plt.suptitle('Optimizer : Adam | Model 4', fontsize=10)
...
def plot_confusion_matrix(cm, classes,
                          normalize=False,
                          title='Confusion matrix | Model 4',
                          cmap=plt.cm.Blues):
    """
...
def show_confusion_matrix(y_test, y_pred, label_map):
    ...
    y_actu.to_csv('test_vector_4.csv',  header=True, index=False)
    y_pred.to_csv('predicted_vector_4.csv',  header=True, index=False)
...
model4 = model4(num_class)
train(model4, X_train, y_train, X_test, y_test)
